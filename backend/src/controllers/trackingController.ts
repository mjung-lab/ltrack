...
const clickSessions = new Map<string, { trackingCodeId: string, timestamp: number }>();

// トラッキングコード生成
export const createTrackingCode = async (req: Request, res: Response) => {
  try {
    ...
    // ユニークなトラッキングコード生成
    const code = `ltk_${uuidv4().replace(/-/g, '').substring(0, 12)}`;
    ...
    logger.info(`Tracking code created: ${code}`, {
      userId: req.user!.userId,
    ...
    logger.error('Create tracking code error:', error);
    res.status(500).json({
...
// トラッキングコード一覧取得
export const getTrackingCodes = async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Get tracking codes error:', error);
    res.status(500).json({
...
// トラッキングコード詳細取得
export const getTrackingCodeDetails = async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Get tracking code details error:', error);
    res.status(500).json({
...
export const recordClick = async (req: Request, res: Response) => {
  try {
    ...
    logger.info(`Click recorded and redirecting: ${code} -> ${lineUrl}`);
...
// トラッキングコード更新
export const updateTrackingCode = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { name, description, lineAccountId } = req.body;
    ...
    logger.info(`Tracking code updated: ${id}`, {
      userId: req.user!.userId
    });
    ...
    logger.error('Update tracking code error:', error);
    res.status(500).json({
...
// トラッキングコード削除
export const deleteTrackingCode = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    logger.info(`Tracking code deleted: ${id}`, {
      userId: req.user!.userId
    });
    ...
    logger.error('Delete tracking code error:', error);
    res.status(500).json({
...
// セッション情報を取得する関数
export const getTrackingSession = (sessionId: string) => {
