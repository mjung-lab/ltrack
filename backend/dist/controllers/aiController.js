...
exports.getChurnPrediction = exports.getSegmentAnalysis = exports.getROIPrediction = exports.getFriendAdditionPrediction = void 0;
const client_1 = require("@prisma/client");
const app_1 = require("../app");
const prisma = new client_1.PrismaClient();
// 友だち追加予測
const getFriendAdditionPrediction = async (req, res) => {
    try {
    ...
    `;
        ...
        app_1.logger.error('Get friend addition prediction error:', error);
        res.status(500).json({
...
exports.getFriendAdditionPrediction = getFriendAdditionPrediction;
// ROI予測
const getROIPrediction = async (req, res) => {
    try {
        ...
        app_1.logger.error('Get ROI prediction error:', error);
        res.status(500).json({
...
exports.getROIPrediction = getROIPrediction;
// セグメント分析（RFM分析）
const getSegmentAnalysis = async (req, res) => {
    try {
        // 友だち追加データの分析
        ...
        app_1.logger.error('Get segment analysis error:', error);
        res.status(500).json({
...
exports.getSegmentAnalysis = getSegmentAnalysis;
// チャーン予測
const getChurnPrediction = async (req, res) => {
    try {
        ...
        app_1.logger.error('Get churn prediction error:', error);
        res.status(500).json({
...
// レコメンデーション生成関数
function generateRecommendations(conversionRate, growthRate) {
...
function generateROIRecommendations(roi, cpa) {
    const recommendations = [];
    if (roi < 100) {
...
function generateChurnRecommendations(churnRate, riskSummary) {
    const recommendations = [];
    if (churnRate > 20) {
